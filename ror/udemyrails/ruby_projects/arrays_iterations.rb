...
# Some methods you can use on arrays:
    ...
    arrayname.unshift(27) # will add element "some element" to the beginning of the array
...
arrayNameString = ["Zeaph", "Buddy", "Ely", "Marcus", "Raymond", "Ebe"]

arrayNameString.each do |randomvariablename| # starts the do block
...
# To capitalize (or use another method) on each element of array called names:
    arrayNameString.each { |randomvariablename| puts "Hello #{randomvariablename.capitalize}" }

# Using the select method to pickup all the odd numbers from an array arrayname:
    arrayname.select { |number| number.odd? }
    # selects the value and returns it only if the condition is met

# To join the elements of an array named arrayNameString:
puts arrayNameString.join
